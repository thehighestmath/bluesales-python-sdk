    ...
    def send(self, method: str, data: dict | list | None = None) -> dict | list:
        ...
        try:
            result = requests.post(
                url=self.__base_url, params=payload, data=json.dumps(data)
            )
        ...
        if (
            ...
            and "isValid" in response
            and not response["isValid"]
        ):
            ...
            if "Другой пользователь находится онлайн под логином" in error_msg:
                ...
                p2 = "</span>"
                delay = int(
                    error_msg[error_msg.index(p1) + len(p1) : error_msg.index(p2)]
                )
                time.sleep(delay + 1)
